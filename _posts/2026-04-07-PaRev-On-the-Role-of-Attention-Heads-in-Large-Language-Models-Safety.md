...
description: Attention heads play a critical role in shaping both safe and harmful behaviors
...
$$
\begin{equation}
\arg\min_\theta - \log p(R_\perp | x_\mathcal{H} = x_1, x_2, \dots, x_s ; \theta)
\end{equation}
$$
...
![alt text](/assets/img/posts/Ships_Sahara/1_algo.png){: width = 200}
...
본 연구에서 제안한 기법을 사용하면 **<span style='color: #6a5acd'>굉장히 소수(0.018%, attention head 3개)의 parameter를 수정함으로써, 큰 ASR의 변화가 나타남을 확인</span>**하였다.
...
Section 5에서는 **Safety Attention Head에 대한 기능을 더욱 깊게 분석**한다.
...
여기서 주목할 점은, **<u>각 attention head에서 Query$(W_q)$, Key$(W_k)$, Value$(W_v)$ matrix들의 변경이 model safety에 어떤 중요성이 있는지</u>** 하는 것이다.
...
이는 **<span style='color: #6a5acd'>attention mechanism의 safety 기능이 alignment 과정에서 학습되는 것이 아니라, pre-training 단계를 통해 형성</span>**되는 것임을 시사한다.
